...
class ManageRecipeView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if listing.is_valid():
                if Recipe.objects.filter(slug=listing.validated_data['slug']).exists():
                    return Response(
    ...
    def patch(self, request):
        try:
            ...
            if not Recipe.objects.filter(contributor_email=user.email, recipe_slug=slug).exists():
                return Response(
                    {'error': 'Listing does not exist'},
                    status=status.HTTP_404_NOT_FOUND
                )

            Recipe.objects.filter(contributor_email=user.email, recipe_slug=slug).update(
                is_published=is_published
            )
    ...
    def delete(self, request):
        try:
            ...
            Recipe.objects.filter(contributor_email=user.email, recipe_slug=recipe_slug).delete()
    ...
    def put(self, request):
        try:
            ...
            origListing = Recipe.objects.get(recipe_slug=request.data['recipe_slug'])
...
class ListRecipeView(ListAPIView):
    queryset = Recipe.objects.order_by('-date_created').filter(is_published=True)
    ...
    def post(self, request):
        try:
            ...
            if not Recipe.objects.filter(pk=data['recipe']).exists():
                ...
                if not LikedRecipes.objects.filter(recipe=recipe, does_liked=does_like, user_email=user.email).exists():
                    
                    liked_data = LikedRecipes.objects.create(recipe=recipe, recipe_id=data['recipe'], does_liked=does_like, user_email=user.email, user=user)
